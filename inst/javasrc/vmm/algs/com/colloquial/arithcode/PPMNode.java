...
public final class PPMNode {
	...
	*/
     ...
     */
    public void interval(int i, ByteSet excludedBytes, int[] result) {
...
/*@ron old code	return hasDaughter(Converter.integerToByte(i));*/
      /*@ron new code*/
      for (PPMNode dtrNode = _firstChild; dtrNode != null; dtrNode = dtrNode._nextSibling)
            if (dtrNode._byte == i) return true;
        return false;
    }
     ...
     */
    void complete(/*@ron byte*/int[] bytes, int offset, int length) {
	PPMNode node = this;
	while (length > 0) {
     ...
     */
    void increment(/*@ron byte*/int [] bytes, int offset, int length) {
	if (_firstChild == null) {
	    ++_numberOfOutcomes;
